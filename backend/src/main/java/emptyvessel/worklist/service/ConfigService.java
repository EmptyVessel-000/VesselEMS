...
import emptyvessel.worklist.repository.ConfigRepository;
...
public class ConfigService {
    ...
    public Config updateConfig(Long id, Config updated) {
        ...
        if (updated.getConfigValue() != null) config.setConfigValue(updated.getConfigValue());
        if (updated.getConfigType() != null) config.setConfigType(updated.getConfigType());
        if (updated.getDescription() != null) config.setDescription(updated.getDescription());
        if (updated.getSortOrder() != null) config.setSortOrder(updated.getSortOrder());
        if (updated.getStatus() != null) config.setStatus(updated.getStatus());
        return configRepository.save(config);
    }
